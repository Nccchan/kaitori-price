...
### 送料無料キャンペーン
- 適用条件：30箱以上かつヤマト運輸利用
- 全国（北海道・沖縄・離島を除く）：着払い発送可・送料無料（九州・四国含む）
- 北海道・沖縄・離島：送料お客様負担（元払い）
...
| 依頼 | 対応ファイル | 変更内容 |
|------|-------------|---------|
| 変更バナーが申請後に見えない問題を修正 | `app.js` | ①`closeCheckoutModal()`でバナー表示中なら`window.scrollTo(0,0)`してページ先頭へ自動スクロール。②`showModificationBanner()`で`showLineExpiredBanner`によるinnerHTML破壊後も「変更する」ボタンを含むバナー構造を再構築して正しく表示する |
